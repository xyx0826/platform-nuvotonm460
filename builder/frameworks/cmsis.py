...
CMSIS_DEVICE_DIR = platform.get_package_dir("framework-cmsis-" + mcu[0:3] + "0")
...
def generate_ldscript(default_ldscript_path):
    ...
    template_file = os.path.join(SOURCES_PATH, "GCC", "gcc_arm.ld")
    content = ""
    with open(template_file) as fp:
        # Replace parentheses around placeholders with braces
        template = re.sub(r"\$\((.+)\)", r"${\1}", fp.read())
        data = string.Template(template)
        content = data.substitute(
            CMramSize=str(int(ram / 1024)),  # K
            CMflashSize=str(int(flash / 1024)))  # K

    with open(default_ldscript_path, "w") as fp:
        fp.write(content)


def get_linker_script(src_path):
    default_ldscript = os.path.join(src_path, "GCC", mcu[0:4] + "_DEFAULT.ld")

    if not os.path.isfile(default_ldscript):
        print("Generating linker script for %s for linking firmware" % mcu)
...
if not board.get("build.ldscript", ""):
    env.Replace(LDSCRIPT_PATH=get_linker_script(SOURCES_PATH))
...
env.BuildSources(
    os.path.join("$BUILD_DIR", "FrameworkCMSIS"), CMSIS_DEVICE_DIR,
    src_filter=[
        "-<*>",
        "+<Source/%s>" % board.get("build.cmsis.system_file", "system_%s.c" % (mcu[0:3] + "0")),
        "+<Source/GCC/%s>"
        % board.get(
